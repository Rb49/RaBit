...
class Application:
    ...
    def __init__(self):
        ...
        Application._wait_for_loading(myClient, loading_window)

        # start the main window
        main_window = get_MainWindow()()
        threading.Thread(target=lambda: asyncio.run(Application._gui_updates(myClient, main_window)), daemon=True).start()
    ...
    @staticmethod
    async def _gui_updates(client: Client, main_window: get_MainWindow()):
        first = dict()
        while True:
