...
class ZoomableMapFrame(customtkinter.CTkCanvas):
    ...
    def __init__(self, master, addresses, **kwargs):
        ...
        self.image = Image.open(ZoomableMapFrame.MAP_PATH).convert('RGBA')
    ...
    def show_image(self):
        ...
        size = (int(max(25, 25 * self.zoom_level)), int(max(33, 33 * self.zoom_level)))
        for pin in self.addresses:
            pin: Tuple[str, str, float, float]  # city, country code, latitude, longitude
            if None in pin[2:4]:
                continue
            if pin[1] is not None:
                pin_image = Image.open(os.path.join(ZoomableMapFrame.PINS_PATH, f"{pin[1]}.png")).convert('RGBA')
            else:
                pin_image = Image.open(os.path.join(ZoomableMapFrame.PINS_PATH, "NONE.png")).convert('RGBA')
            pin_image = pin_image.resize(size)
            position = ZoomableMapFrame.format_coords(pin[2], pin[3], *size, self.prev_width, self.prev_height)
    ...
    def zoom(self, event):
        if event.delta > 0 and self.zoom_level < 1.6:
